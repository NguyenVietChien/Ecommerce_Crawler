...
class ShopeeSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for responses in product:
            ...
            product_discount = responses.css("div.percent").css("::text").extract();
            product_link = "https://shopee.vn"+ str(responses.css('a::attr(href)').get())
        ...
        page_total = response.css(".shopee-mini-page-controller__total::text").get()
        print(numpage+ "------------------------------------------------" + str(page_total))
        next_page ="https://shopee.vn/search?keyword=%C4%91%E1%BB%93%20ch%C6%A1i&page=" + numpage+ "&trackingId=searchhint-1648921902-8e474d9a-b2ad-11ec-a003-08f1ea7b4e082"
      
        # if  len(product.extract()) != 0: 
        if  ShopeeSpider.page_number < int(page_total):
