...
class TipySpider(CrawlSpider):
    name = 'tipy'
    start_urls = ['https://tiki.vn/search?q=%C4%91%E1%BB%93+ch%C6%A1i']
    ...
    rule_product_detail = Rule(tiki_product_detail, callback='parse_item', follow=False)
    ...
    def parse_item(self, response):
        ...
        product_price= response.css(".product-price__current-price::text").get()
        ...
        if title is not None:
            ...
            items['product_price'] = convert_price(product_price)
            # items['product_discount'] = product_discount

            items['rating_point'] = convert_rating_total(product_review_rating_point)
            items['total_comments'] = convert_comment(product_comments)
            
            items['rating_5_star'] = convert_rating(product_review_rating_5_star),
            items['rating_4_star'] = convert_rating(product_review_rating_4_star),
            items['rating_3_star'] = convert_rating(product_review_rating_3_star),
            items['rating_2_star'] = convert_rating(product_review_rating_2_star),
            items['rating_1_star'] = convert_rating(product_review_rating_1_star),
            items['platform'] = 'tiki'
...
def convert_comment(comment):
    if comment is not None:
        return int(comment.replace(" nhận xét", ""))
    else:
        return 0
